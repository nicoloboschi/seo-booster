...
### Zep's Managed Cloud Service: Pricing Tiers and Zep Cloud Pricing
...
These tiered plans make the service accessible for various scales, from small projects to enterprise-level deployments. For precise details on current pricing, it's always best to consult the [official Zep documentation](https://docs.getzep.com/getting-started/installation/). Understanding the **Zep memory cost** is essential for planning. For those looking for a **Zep cloud pricing free tier**, while the open-source is free, the managed cloud service is consumption-based.
...
This is crucial for applications like **AI assistants that remember conversations**, **agentic AI long-term memory**, and **long-term memory AI chat** systems. Explore [AI agent memory explained](/articles/ai-agent-memory-explained/) for a foundational understanding.
...
These different memory types contribute to a more holistic **AI agent persistent memory** solution. For a deeper dive into memory types, see [AI agents' memory types](/articles/ai-agents-memory-types/).
...
## Assuming Zep is running locally on default port 8000
## For cloud service, you would provide api_key and api_url
## The question of 'is Zep memory free' is answered by self-hosting this.
memory = ZepMemory(url="http://localhost:8000")
...
## The memory object can now be inspected or used to retrieve past interactions
...
The open-source landscape for AI memory is growing. Projects like **Hindsight**, an open-source AI memory system, offer self-hostable solutions that provide flexibility and cost savings, similar to the free option of Zep. Hindsight, found on [GitHub](https://github.com/vectorize-io/hindsight), allows developers to build custom memory backends and integrate them with LLM applications.

Other notable open-source contenders include:

* **Mem0:** Known for its efficient storage and retrieval of LLM memories. You can find comparisons in [Mem0 alternatives](/articles/mem0-alternatives-compared/).
* **Letta AI:** Focuses on providing a structured approach to AI memory management. See our [Letta AI guide](/articles/letta-ai-guide/).

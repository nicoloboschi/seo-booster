...
- question: What's the difference between an AI's context window and its long-term memory?
  answer: An AI's **context window** is a temporary, fixed-size buffer for processing current information. **Long-term memory** is a persistent storage and retrieval system designed to retain information
    over extended periods, far beyond the context window's limits. This distinction is key for understanding the **best AI with long memory**.
- question: How can I improve an AI's ability to remember past interactions?
  answer: Improve AI memory using techniques like Retrieval-Augmented Generation (RAG), integrating vector databases, employing memory consolidation strategies, or utilizing specialized **AI memory systems**
    designed for persistent recall. These methods contribute to building an AI with excellent long memory.
...
## Simulate a memory store, akin to a vector database.
## It will hold the numerical representations of our memories.
memory_store = []
memory_texts = [] # To store the original text for retrieval display

def add_memory(text_content):
 """Adds a new memory entry to the store after converting it to an embedding."""
...
def retrieve_memories(query_text, top_n=3):
 """Retrieves the most similar memories to a given query using embeddings."""
 query_embedding = model.encode(query_text)

 # Calculate cosine similarity between the query embedding and all memory embeddings.
 # This measures how semantically similar the query is to each stored memory.
 similarities = cosine_similarity([query_embedding], memory_store)[0]

 # Get indices of the top_n most similar memories based on similarity scores.
 ...
 print(f"- '{text[:50]}...' (Score: {score:.4f})")
 return retrieved_memories

## Example usage: Adding distinct pieces of information to the AI's memory.
add_memory("The user asked about the weather yesterday at 3 PM.")
add_memory("The AI recommended a restaurant last week, and the user enjoyed it.")
add_memory("The user's preferred programming language is Python, especially for data science.")
add_memory("The AI explained the concept of RAG in detail during our last session, focusing on vector databases.")
